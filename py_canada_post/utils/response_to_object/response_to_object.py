...
def is_date_valid(date: str) -> datetime | None:
    try:
...
class ResponseToObject:
    ...
    def _construct_object(self, obj: dict, keys: list[KeyPath], cls: Type[T]) -> T:
        ...
        for key in keys:
            ...
            elif isinstance(nested_key, str) and  is_date_valid(nested_key):
                data.append(is_date_valid(nested_key))
    ...
    def _construct_objects(self, obj: dict, search_keys: list[KeyPath], obj_keys: list[KeyPath], cls: Type[T]) -> list[T] | None:
        """
